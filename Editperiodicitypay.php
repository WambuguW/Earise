                ...
                <div class="col-md-offset-1 col-md-10 col-sm-10">
                    ...
                    <div class="portlet box green calendar">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="icon-reorder"></i> Periodicity Edit
                            ...
                            <div class="well dark_green">
                                ...
                                <div class="art-layout-cell art-content clearfix">
                                    ...
                                    if(isset($_REQUEST['pedit'])){
                                        edditedperiodEdit($_REQUEST['pedit']);
                                        $id = ($_REQUEST['pedit']);
                                        $stmt = mysql_query("SELECT * FROM  periodictyrecord  WHERE   id='$id'");
                                         while ($row = mysql_fetch_array($stmt)) {
                                         $oldname= base64_decode($row['periodname']);
                                         $oldperiod= base64_decode($row['numberdays']);
    }
                                    }
                                    if(isset($_REQUEST['periodupdate'])){
                                        updateperiod($_REQUEST['pedit'],$_REQUEST['period'],$_REQUEST['freq'], $oldname, $oldperiod);
                                    }
                                    if(isset($_REQUEST['pdel'])){
                                      include ('del.php');   
                                    }
                                    if(!isset($_REQUEST['pedit']) && !isset($_REQUEST['periodupdate']) && !isset($_REQUEST['pdel']))
                                    {
                                    periodicEdit();
                                    }
                                    ?>
