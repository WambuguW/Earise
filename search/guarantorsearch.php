...
function loanrepaymentreport($user, $q) {    
    ...
    echo '<table class="tables">
        ...
        <tr><td>' . getsymbol() . '.'.  number_format(base64_decode($loanrslt['amount'])+  totalloanint($mrslt['membernumber']),2).'</td>
            <td>' . getsymbol() . '.'.  number_format(base64_decode($loanrslt['monthlypayment']),2).'</td>
                <td>' . getsymbol() . '.'. number_format(addloanamoutpaid($mrslt['membernumber']),2).'</td>
                <td>' . getsymbol() . '.'.  number_format(remailingloan($mrslt['membernumber']),2).'</td>
