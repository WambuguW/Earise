    ...
    function amort($amount = 0, $rate = 0, $years = 0) {
        ...
        if ($amount * $rate * $years > 0) {
            ...
            if (loantyper($_SESSION['ltid']) == '2') {
                ...
                $this->pmnt = $amount / ( $this->npmts); //monthly payment
                $this->tpmnt = $amount; //total amount paid at end of loan
...
//    displays: everything for false case plus the amortization table
    ...
    function showTable() {
        ...
        for ($pnum = 1; $pnum <= $this->npmts; $pnum++) {
            ...
            $bbal = $ebal;
                print '<input type="hidden" name="start_bal[]" value="' . round($bbal) . '">' . "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($bbal)) . "</td>";
            ...
            if (loantyper($_SESSION['ltid']) == '2') {
                ...
                if ($this->mrate == 0) {
                    ...
                    print'<input type="hidden" name="interest_payment[]" value="' . round($ipmnt) . '">' . "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($ipmnt)) . "</td>";
                    print '<input type="hidden" name="principal_payment[]" value="' . round($ppmnt) . '">' . "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($ppmnt)) . "</td>";
                    $ebal = $bbal - $ppmnt;
                    print '<input type="hidden" name="end_bal[]" value="' . round($ebal) . '">' . "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($ebal)) . "</td>";
                    $ccint = $ccint + $ipmnt;
                    print '<input type="hidden" name="cumulative_interest[]" value="' . round($ccint) . '">' . "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($ccint)) . "</td>";
                   $ttotal = $ipmnt + $ppmnt;
                    print "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($ttotal), 2) . "</td>";
                    $cpmnt = $cpmnt + $this->pmnt;
                    print '<input type="hidden" name="cumulative_payment[]" value="' . round($cpmnt) . '">' . "<td width='14%' align='right'>" . getsymbol() . " " . number_format(round($cpmnt)) . "</td>";
