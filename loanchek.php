                        ...
                        <div class="col-md-offset-1 col-md-9 col-sm-9">
                            ...
                            <div class="portlet box green calendar">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class="icon-reorder"></i> Pending Loan Approvals
                                        ...
                                        <div class="noprint">
                                            ...
                                            <?php
                                                ...
                                                if (isset($_REQUEST['reject'])) {
                                                    ...
                                                    $sapy = mysql_query('update loanapplication set status="' . base64_encode('rejected') . '" where transactionid="' . base64_encode($_REQUEST['tid']) . '"') or die(mysql_error());

                                                    if ($sapy) {
                                                        echo '<span class="green" >The Loan Application from ' . getMembername(base64_encode($_REQUEST['mno'])) . ' Has Been Rejected.</span>';
                                                    }
                                                }

                                                if (isset($_REQUEST['update'])) {
                                                     $_SESSION['ltid'] = $_REQUEST['tid'];  //transaction id
                                                            $_SESSION['lmid'] = $_REQUEST['mno'];  //member number
                                                            $_SESSION['ltype'] = $_REQUEST['ltype'];   //loan type //
                                                            $_SESSION['amtt'] = $_REQUEST['amount'];   // amount 
                                                            $_SESSION['norep'] = $_REQUEST['years'];   // no of repayment
                                                            $_SESSION['appdate'] = $_REQUEST['appdate'];  //application date
                                                            $_SESSION['gperiod'] = $_REQUEST['gperiod'];  //grace period
                                                            $_SESSION['mode'] = $_REQUEST['mode'];   // mode of payment weekly/mothnly/daily
                                                    ...
                                                    if (!$amount = $_POST['amount']) { //first time set all to zero
                                                        ...
                                                        $am = new amorta($amount, $rate, $years); //make an instance of amort and set the numbers
                                                        $am->showsForm();  //show the form to get the numbers
                                                        if ($amount * $rate * $years <> 0) { //if any one is zero, don't show the table
                                                            $am->showaTable();  //if true, show table, else don't
                                                        ...
                                                        echo "<input type='submit'  class='mybutton' name='ssdate' value='Check Schedule.' align='middle'/>"
                                                            . "  <button class='mybutton' name='process' value='process'>Process Loan</button></form>";
    
                                                }
                                                 

                                                if (isset($_REQUEST['chek'])) {
                                                    ...
                                                    updateloan($_SESSION['users'], $rows['id'], base64_decode($rows['membernumber']), base64_decode($rows['loantype']), base64_decode($rows['installments']), base64_decode($rows['amount']), base64_decode($rows['transactionid']),base64_decode($rows['paymode']),base64_decode($rows['gperiod']),base64_decode($rows['appdate']));
                                                }
                                                

                                                if (!isset($_REQUEST['chek']) && !isset($_REQUEST['update'])) {
                                                    loanchecka();
                                                }
