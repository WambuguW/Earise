                        ...
                        <div class=" col-md-offset-1 col-md-11">
                            ...
                            <div class="portlet box green calendar">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class="icon-reorder"></i> Issue Invoice
                                        ...
                                        <div class="art-layout-cell art-content clearfix">
                                            ...
                                            <?php
                                                ...
                                                if (isset($_REQUEST['create'])) {
                                                    ...
                                                    $newuser->addissuedinvoice($_SESSION['users'], $_REQUEST['debtor'], $_REQUEST['dateinvoice'], $_REQUEST['glacc'], $_REQUEST['amount'], $_REQUEST['invono'], $_REQUEST['duedate'], $_REQUEST['description'], $_REQUEST['status']);
                                                }

                                                if (isset($_REQUEST['gid']) || isset($_REQUEST['gedit']) || isset($_REQUEST['save']) || isset($_REQUEST['issdel'])) {

                                                    if (isset($_REQUEST['gid'])) {

                                                        $id = $_REQUEST['gid'];

                                                        $sql = mysql_query('SELECT * FROM issueinvoice WHERE id="' . $id . '"') or die(mysql_error());

                                                        $rows = mysql_fetch_array($sql);

                                                        invoicedit($rows['id'], base64_decode($rows['debtorid']), base64_decode($rows['invdate']), base64_decode($rows['glacc']), base64_decode($rows['amount']), base64_decode($rows['invno']), base64_decode($rows['duedate']), base64_decode($rows['status']), base64_decode($rows['description']));
                                                    }

                                                    if (isset($_REQUEST['save'])) {
                                                        issinvoiceupdate($_SESSION['users'], $_REQUEST['id'], $_REQUEST['debtor'], $_REQUEST['invdate'], $_REQUEST['glacc'], $_REQUEST['amount'], $_REQUEST['invno'], $_REQUEST['duedate'], $_REQUEST['status'], $_REQUEST['description']);
                                                    }
