                        ...
                        <div class="col-md-offset-1 col-md-10 col-sm-10">
                            ...
                            <div class="portlet box green calendar">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class="icon-reorder"></i> Expenses Edit
                                    ...
                                    <div class="well dark_green">
                                        ...
                                        <div class="art-layout-cell art-content clearfix">
                                            ...
                                            <?php
                                                ...
                                                if (isset($_REQUEST['edit']) || isset($_REQUEST['btnedit']) || isset($_REQUEST['btnupdate'])   || isset($_REQUEST['edel'])   ) {

                                                    if (isset($_REQUEST['btnedit'])) {

                                                        $id = $_REQUEST['xid'];

                                                        $sql = mysql_query('SELECT * FROM paymentout WHERE primarykey="' . $id . '"') or die(mysql_error());

                                                        $rows = mysql_fetch_array($sql);

                                                        expensedit($_SESSION['users'], $rows['primarykey'], base64_decode($rows['receiver']), base64_decode($rows['receiverid']), base64_decode($rows['approvedby']), base64_decode($rows['date']), base64_decode($rows['reasons']), base64_decode($rows['chequeno']), base64_decode($rows['amount']), base64_decode($rows['comments']), base64_decode($rows['transid']),base64_decode($rows['bnkid']),base64_decode($rows['paymentype']) ,base64_decode($rows['session']));
                                                    }
                                                    if (isset($_REQUEST['btnupdate'])) {
                                                        expupdate($_SESSION['users'], $_REQUEST['id'], $_REQUEST['recvr'], $_REQUEST['recvrid'], $_REQUEST['apby'], $_REQUEST['date'], $_REQUEST['reason'], $_REQUEST['tname'], $_REQUEST['ptype'], $_REQUEST['cheque'], $_REQUEST['res'], $_REQUEST['comments'], $_REQUEST['tid'], $_REQUEST['session'],$_REQUEST['bankname']);
                                                    }

                                                    if (isset($_REQUEST['edel'],$_REQUEST['amount'],$_REQUEST['date'])) {
                                                        include ('del.php');
                                                    }
                                                }  else {
                                                    expedit();
