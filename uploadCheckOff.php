...
<html lang="en" class="no-js">
                        ...
                        <div class="col-md-10 col-md-offset-1 col-sm-10">
                            ...
                            <div class="portlet box green calendar">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class="icon-reorder"></i>Upload Check  Off
    ...
    do { 
        if ($data[0]) {
            $mnth = strtotime(addslashes($data[2]));
            $newdate = date('M-Y', $mnth);
            $date=date('d-M-Y',strtotime($data[5]));
            if(strlen($data[3])>=3){
            
           $tid=loantid($data[0], getloanid(getglacc($data[3])));
            }else{
                $tid= base64_encode($data[3]);
            }
            
            $stmt=  mysql_query("SELECT * FROM check_off  WHERE   mno='".base64_encode(addslashes($data[0]))."' AND   org='".base64_encode(addslashes($data[1]))."'  AND  mnth='".base64_encode(($newdate))."'   ");
            if(mysql_num_rows($stmt)==0){
                
           $sth =mysql_query("INSERT INTO check_off (mno, org, mnth, tid, amount, date,status) VALUES 
                ( 
                    '". base64_encode(addslashes($data[0]))."', 
                    '". base64_encode(addslashes($data[1]))."', 
                    '". base64_encode(($newdate))."',
                    '".$tid."',
                    '". base64_encode(addslashes($data[4]))."',
           ...
           if($sth){
            ...
            echo "<script> alert('$filename failed to imported File Because File Already Exits')</script>"; 
        }
        } 
